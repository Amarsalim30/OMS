...
3. Device binding:
   - Each user has `maxDevices` (default 1 or 2).
   - First run on a device registers that device.
   - If device limit exceeded, app blocks with a clear reason.
4. Admin control:
   - Admin can allow/deny users.
   - Admin can revoke a specific device for a user.
5. Minimal infrastructure:
   - Use Firebase Auth (Google Sign-In via Credential Manager).
...
7. UX:
   ...
   - If signed out, show login screen.
8. Offline policy:
...
2. Same user on second device when maxDevices=1 -> blocked: "Device limit reached".
3. Revoked device -> blocked on next validation.
