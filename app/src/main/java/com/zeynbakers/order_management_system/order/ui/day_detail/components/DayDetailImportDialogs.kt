...
internal fun DayImportPreviewDialog(
    ...
    val importDate =
        runCatching { kotlinx.datetime.LocalDate.parse(importData.orderDate) }.getOrNull()
                ...
                if (isCrossDayImport) {
                    Surface(
                        ...
                        Column(modifier = Modifier.padding(12.dp)) {
                            ...
                            Text(
                                text = stringResource(
                                    R.string.day_import_date_adjustment_message,
                                    importDate,
                                    currentDate
                                ),
            ...
            if (item.cartItems.isNotEmpty()) {
                Column(modifier = Modifier.padding(top = 4.dp)) {
                    ...
                    if (item.cartItems.size > 3) {
                        Text(
                            text = stringResource(
                                R.string.day_import_more_items,
                                item.cartItems.size - 3
                            ),
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant
                        )
