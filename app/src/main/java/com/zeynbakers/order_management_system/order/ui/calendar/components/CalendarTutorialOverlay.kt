        ...
        Canvas(
            ...
            targetBounds?.let { target ->
                ...
                drawRoundRect(
                    ...
                    topLeft = Offset(
                        (left - pulseInset).coerceAtLeast(0f),
                        (top - pulseInset).coerceAtLeast(0f)
                    ),
                    size = Size(
                        (width + (pulseInset * 2)).coerceAtMost(
                            size.width - (left - pulseInset).coerceAtLeast(
                                0f
                            )
                        ),
                        (height + (pulseInset * 2)).coerceAtMost(
                            size.height - (top - pulseInset).coerceAtLeast(
                                0f
                            )
                        )
                    ),
