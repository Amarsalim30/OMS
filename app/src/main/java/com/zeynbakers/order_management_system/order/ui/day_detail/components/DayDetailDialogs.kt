package com.zeynbakers.order_management_system.order.ui.day_detail.components
...
internal fun DayOrderEditorDialog(
    ...
    val orderDateLabel =
        remember(orderDate) {
            DateTimeFormatter.ofPattern("d MMM", Locale.getDefault())
                .format(orderDate.toJavaLocalDate())
        }
    ...
    val remaining =
        if (currentTotal > paidAmount) currentTotal.subtract(paidAmount) else BigDecimal.ZERO
    ...
    OrderEditorSheet(
        ...
        totalSupportingText = when {
            ...
            formattedTotal != null -> stringResource(
                R.string.day_editor_total_preview,
                formattedTotal
            )
            ...
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                if (!hasAllocations) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        TextButton(
                            onClick = {
                                onSetDeleteSelection(deleteAllocations.map { it.allocationId }
                                    .toSet())
                            }
                        ) { Text(stringResource(R.string.day_select_all)) }
                        TextButton(onClick = { onSetDeleteSelection(emptySet()) }) {
                            Text(
